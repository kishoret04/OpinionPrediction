...
class DynamicFPModel(pl.LightningModule):
    ...
    def forward(self,  author, read_track, write_track, article_pack, sentiments, emotion):
        ...
        test_fp_out = {}        
        ######analysis save########
        test_fp_out['author'] = author.detach().cpu().numpy()
        test_fp_out['read_track_0'] = read_track[0].detach().cpu().numpy()
        test_fp_out['read_track_1'] = read_track[1].detach().cpu().numpy()
        test_fp_out['write_track_0'] = write_track[0].detach().cpu().numpy()
        test_fp_out['write_track_1'] = write_track[1].detach().cpu().numpy()
        ########################
        ...
        ######analysis save########
        test_fp_out['track_ht_read'] = track_ht_read.detach().cpu().numpy()
        test_fp_out['track_ht_write'] = track_ht_write.detach().cpu().numpy()
        test_fp_out['author_embeds'] = author_embeds.detach().cpu().numpy()
        test_fp_out['result'] = ls_train_sent.detach().cpu().numpy()
        test_fp_out['truth'] = labels.detach().cpu().numpy()
        # self.dict_test_fp_out[self.index_test_fp_out] = test_fp_out
        # self.index_test_fp_out+=1
        #####################
        
        return ls_train_sent, labels, test_fp_out
    ...
    def on_test_epoch_start(self):
        ##Analysis
        self.list_test_fp_out = []
        # self.index_test_fp_out = 0
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.config['emotion_fingerprinting']:
            ...
            emotion = np.NaN
        test_result, labels,test_fp_out = self(author, r_tracks, w_tracks, articles, sentiment, emotion)#, self.device)
        ...
        #analysis
        self.list_test_fp_out.append(test_fp_out)
    ...
    def on_test_epoch_end(self):
        ...
        ##analysis file save
        dict_test_fp_out = {}
        for i, each_batch in enumerate(self.list_test_fp_out):
            dict_test_fp_out[i] = each_batch

        df_test_fp_out = pd.DataFrame.from_dict( dict_test_fp_out, orient='index')
        test_fp_out_file = 'test_fp_out' +dt_string + '.pkl' 
        folder_path = os.path.join( self.config['root_folder'], self.config['outlet'])
        test_fp_out_file = os.path.join( folder_path, 'test_fp_analysis',test_fp_out_file )        
        df_test_fp_out.to_pickle(test_fp_out_file )
        print('Analysis file saved at: ', test_fp_out_file)
        #####
...
class BertFPModel(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        ...
        if config['sentiment_fingerprinting']:
            self.vader_predict = nn.Sequential(
                nn.Linear(in_dim, config['sentiment_dim']),
                nn.ReLU(),
                # nn.Linear(config['sentiment_dim'], config['sentiment_dim']),
                # nn.ReLU(),
                nn.Linear(config['sentiment_dim'], 3))
            self.flair_predict = nn.Sequential(
                nn.Linear(in_dim, config['sentiment_dim']),
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        avg_acc, avg_f1 = [], []
        for i,m in enumerate(self.val_accuracy):
            self.log('val_acc_'+str(i), m.compute())
            avg_acc.append( m.compute())

        for i,m in enumerate(self.val_f1):
            self.log('val_f1_'+str(i), m.compute())
            avg_f1.append( m.compute())
    ...
    def on_test_epoch_end(self):
        ...
        for i,m in enumerate(self.test_accuracy):
            index = 'test_acc_'+str(i)
            print( index+ ' : ' , m.compute())
            test_result[index] = m.compute().detach().cpu().item()

        for i,m in enumerate(self.test_f1):
            index = 'test_f1_'+str(i)
            print( index+ ' : ' , m.compute())
            test_result[index] = m.compute().detach().cpu().item()
        ...
        json.dump(test_result, open(os.path.join( self.config['root_folder'], self.config['outlet'],dump_file ), 'w'))
        ##analysis file save
        df_test_fp_out = pd.DataFrame.from_dict( self.dict_test_fp_out, orient='index')
        test_fp_out_file = 'test_fp_out' +dt_string + '.pkl' 
        test_fp_out_file = os.path.join( self.config['root_folder'], self.config['outlet'],test_fp_out_file )        
        df_test_fp_out.to_pickle(test_fp_out_file )
        print('Analysis file saved at: ', test_fp_out_file)
        #####
    ...
    def _bert_encoder(self, seq_seq_tensor, token_mask):
        ...
        # Feed input to BERT/ELECTRA
        if self.config['sel_model'] == 'electra':
            model_outputs = self.electra(input_ids= token_encode_mtx, attention_mask= attention_mtx)
        else:
            model_outputs = self.bert(input_ids= token_encode_mtx, attention_mask= attention_mtx)
